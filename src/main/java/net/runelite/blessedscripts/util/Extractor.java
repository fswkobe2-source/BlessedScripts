package net.runelite.blessedscripts.util;

import lombok.extern.slf4j.Slf4j;
import net.runelite.blessedscripts.internal.globval.GlobalConfiguration;
...
public class Extractor implements Runnable {
	...
	public void run() {
        ...
        if (GlobalConfiguration.getCurrentOperatingSystem() == GlobalConfiguration.OperatingSystem.WINDOWS) {
          Extractor.saveTo(loader.getResourceAsStream(root + GlobalConfiguration.Paths.COMPILE_SCRIPTS_BAT), GlobalConfiguration.Paths.getBlessedScriptsDirectory() + File.separator + GlobalConfiguration.Paths.COMPILE_SCRIPTS_BAT);
          Extractor.saveTo(loader.getResourceAsStream(root + GlobalConfiguration.Paths.COMPILE_FIND_JDK), GlobalConfiguration.Paths.getBlessedScriptsDirectory() + File.separator + GlobalConfiguration.Paths.COMPILE_FIND_JDK);
				} else {
					Extractor.saveTo(loader.getResourceAsStream(root + GlobalConfiguration.Paths.COMPILE_SCRIPTS_SH), GlobalConfiguration.Paths.getBlessedScriptsDirectory() + File.separator + GlobalConfiguration.Paths.COMPILE_SCRIPTS_SH);
				}
